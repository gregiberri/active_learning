...
class Metrics(object):
    ...
    def draw_goal_metric_by_class(self, epoch_pred, epoch_gt, writer, epoch):
        ...
        # get the class names, numbers and the goal metrics by class
        with open(os.path.join('data/data_parts', 'batches.meta'), 'rb') as f:
            class_names = pickle.load(f, encoding='bytes')[b'label_names']
            class_names = [class_name.decode('ascii') for class_name in class_names]
        goal_metric_by_class = np.array(goal_metric_by_class)
        ...
        df_cm = pd.DataFrame(array.numpy(), class_names, class_names)
