...
def entropy(input):
    return -torch.sum(input * torch.log(input), -1)


def entropy_sampling(pred_path, sample_number):
    with open(pred_path, 'rb') as fo:
        pred_dict = pickle.load(fo, encoding='bytes')
    pred_softmaxes = F.softmax(pred_dict['preds'], -1)
    pred_entropies = entropy(pred_softmaxes)
    _, sample_indices = torch.topk(pred_entropies.cpu(), k=sample_number, dim=0)
    ...
    data_part_path = os.path.join('data', 'data_parts', 'entropy')
...
if __name__ == '__main__':
    ...
    entropy_sampling(pred_path, 5000)
