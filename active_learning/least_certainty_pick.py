...
def uncertainty_sampling(pred_path, sample_number):
    ...
    max_pred_softmaxes, _ = torch.max(pred_softmaxes, dim=-1)
    _, sample_indices = torch.topk(-max_pred_softmaxes.cpu(), k=sample_number, dim=0)
    ...
    data_part_path = os.path.join('data', 'data_parts', 'least_certainty')
